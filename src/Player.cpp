...
Player::Player(sf::Texture& texture_sheet){
    ...
    this->currentFrame = sf::IntRect(0, 0, 170, 158);
    // this->sprite.setScale(5.f, 5.f);
    this->hitbox = sf::FloatRect(0.f, 0.f, 170.f, 158.f);
...
void Player::updateMovement() {
    ...
    // We only change the state to movement/idle if we aren't currently attacking
    // If we are attacking, we let the animation finish first
    if (this->animState != ATTACKING) {
        if (std::abs(velocity.x) > 0.1f) {
            // Both running and walking now trigger the same animation logic
            this->animState = (velocity.x > 0.f) ? MOVING_RIGHT : MOVING_LEFT;
        } else {
            this->animState = IDLE;
        }
    }
...
void Player::updateAnimations() {
    float frameWidth = 170.f;
    float frameHeight = 158.f;

    this->currentFrame.width = frameWidth;
    this->currentFrame.height = frameHeight;

    // Reset frame to 0 if state changes (e.g., just started moving or attacking)
    if (this->animState != this->previousAnimState) {
        this->currentFrame.left = 0.f;
        this->animationClock.restart();
    }
    ...
    if (this->animState == IDLE) {
        this->currentFrame.top = 316.f; // Attack/Idle row
        this->currentFrame.left = 0.f;
    } 
    // --- ATTACK STATE ---
    else if (this->animState == ATTACKING) {
        this->currentFrame.top = 316.f;
        this->currentFrame.left = 170.f; // Attack frame

        // FIX: After 0.3 seconds, return to IDLE so movement can take over again
        if (this->animationClock.getElapsedTime().asSeconds() >= 0.3f) {
            this->animState = IDLE;
        }
    } 
    // --- MOVEMENT STATE (Combined Walking and Running) ---
    else if (this->animState == MOVING_LEFT || this->animState == MOVING_RIGHT || 
             this->animState == RUNNING_LEFT || this->animState == RUNNING_RIGHT) {
        
        this->currentFrame.top = 0.f; // Row 1: Movement
        
        // Loop 2-frame animation
        if (this->animationClock.getElapsedTime().asSeconds() >= 0.15f) {
            this->currentFrame.left += frameWidth;
            if (this->currentFrame.left >= 2 * frameWidth) { 
                this->currentFrame.left = 0.f;
            }
            this->animationClock.restart();
        }

        // Handle Flipping based on current state
        if (this->animState == MOVING_LEFT || this->animState == RUNNING_LEFT) {
            this->sprite.setScale(-1.f, 1.f);
            this->sprite.setOrigin(frameWidth, 0.f);
        } else {
            this->sprite.setScale(1.f, 1.f);
